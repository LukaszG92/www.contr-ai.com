...
function extractCreditiInfo(file: string, percentualeCessione: number, percentualeRevisione: number, percentualeConsulenza: number, annoIniziale: number, annoFinale: number): {
    ...
    sums.forEach( (value, key, map) => {
        ...
        calcoli.set(key, `${value.toFixed(2)}, ${numeroInParole(Number(value.toFixed(2))).toUpperCase()}`)

        let sconto = value * (percentualeCessione/100)
        calcoli.set('sconto'+year, `${sconto.toFixed(2)}, ${numeroInParole(Number(sconto.toFixed(2))).toUpperCase()}`)
        scontoTotale += sconto

        let commissioneR = value * (percentualeRevisione/100) * 1.22
        calcoli.set('commissione revisione'+year, `${commissioneR.toFixed(2)}, ${numeroInParole(Number(commissioneR.toFixed(2))).toUpperCase()}`)
        commissioneRevisione += commissioneR

        let commissioneC = value * (percentualeConsulenza/100) * 1.22
        calcoli.set('commissione consulenza'+year, `${commissioneC.toFixed(2)}, ${numeroInParole(Number(commissioneC.toFixed(2))).toUpperCase()}`)
        commissioneConsulenza += commissioneC

        let netto = sconto - commissioneR - commissioneC
        nettoTotale += netto
        calcoli.set('netto'+year, `${netto.toFixed(2)}, ${numeroInParole(Number(netto.toFixed(2))).toUpperCase()}`)

        calcoli.set('sconto', `${scontoTotale.toFixed(2)}, ${numeroInParole(Number(scontoTotale.toFixed(2))).toUpperCase()}`)
        calcoli.set('commissione revisione', `${commissioneRevisione.toFixed(2)}, ${numeroInParole(Number(commissioneRevisione.toFixed(2))).toUpperCase()}`)
        calcoli.set('commissione consulenza', `${commissioneConsulenza.toFixed(2)}, ${numeroInParole(Number(commissioneConsulenza.toFixed(2))).toUpperCase()}`)
        calcoli.set('netto', `${nettoTotale}, ${numeroInParole(Number(nettoTotale.toFixed(2))).toUpperCase()}`)
    })
